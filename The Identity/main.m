...
% Last Modified by GUIDE v2.5 03-Nov-2019 23:49:52
...
if isempty(bbox)
    axes(handles.axes1)
    imshow(vo);
    msgbox({'NO FACE IN THIS PIC IS FOUND IN DB';'PLEASE SELECT SINGLE FACE IMAGE'},'WARNING...!!!','warn','modal')
    uiwait
...
cd ..
    while 1
        qq = inputdlg('WHAT IS YOUR NAME?','FILL');
        if isempty(qq)
            msgbox({'YOU HAVE TO ENTER A NAME';' ';'YOU CANT CLICK CANCEL'},'INFO','HELP','MODAL')
            uiwait
        else
